<script lang="ts">
	...
	$: movie = getFirstMoviePopular(436270);
	setTimeout(() => {
		isLoading = false;
		content = $movie?.data;
	}, 3000);
			...
			<div class="relative bg-black aspect-ratio-1/1 md:aspect-ratio-3/2 lg:aspect-ratio-25/9">
				<div class="absolute top-0 right-0 lt-lg:left-0 lg:bottom-0 lg:left-1/3 bg-gray-9">
					<img
						...
						width="400"
						height="250"
						class="w-full h-full object-cover bg-gray-9"
					/>
				</div>
				<div
					class="absolute bottom-0 top-0 left-0 px-10 flex-col justify-center from-black
				via-black to-transparent lt-lg:bg-gradient-to-t lt-lg:right-0 lt-lg:p-10 lg:w-2/4 lg:bg-gradient-to-r"
				>
